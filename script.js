...
let chartAnnotationPluginLoaded = false;

// --- Defined Color Themes (Combos) ---
const VLF_THEMES = {
    "ocean": { name: "Ocean (Blue/Orange)", p1: '#00b4d8', p2: '#fca311' },
    "earth": { name: "Earth (Green/Yellow)", p1: '#90be6d', p2: '#f9c74f' },
    "classic": { name: "Classic (Blue/Red)", p1: '#1e90ff', p2: '#dc143c' },
    "cyber": { name: "Cyber (Purple/Pink)", p1: '#9d4edd', p2: '#f72585' }
};

// Resistivity Palettes (3 main colors cycled for multiple lines)
const RESISTIVITY_PALETTES = {
    "primary": { name: "Primary (Green/Blue/Orange)", colors: ['#00b67a', '#0ea5e9', '#f97316', '#8b5cf6', '#ec4899', '#ef4444'] },
    "cool": { name: "Cool Tones (Blue/Teal)", colors: ['#0077b6', '#48cae4', '#90e0ef', '#00b4d8', '#34a0a4', '#1f7a8c'] },
...
try {
    // Check for global variable variations
...
function populateThemeDropdowns() {
    const vSelect = document.getElementById('vlfColorTheme');
    const resSelect = document.getElementById('resistivityColorTheme');

    // Populate VLF Themes
    vSelect.innerHTML = Object.keys(VLF_THEMES).map(key => 
...
function calculateGeometricFactor(arrayType, positions) {
    ...
    switch (arrayType) {
        case 'wenner':
            ...
            break;
        case 'schlumberger':
            const L_schl = Math.abs(p4 - p1) / 2; 
            const a_schl = Math.abs(p3 - p2) / 2; 
...
function parseResistivityData(text) {
    ...
    for (const line of lines) {
        ...
        let apparentResistivity = parseFloat(parts[6]); 
...
/**
 * Utility: Creates a linear gradient across the canvas width.
 */
...
function plotVLFData(input) {
    ...
    const theme = VLF_THEMES[selectedThemeKey] || VLF_THEMES["ocean"];
    ...
    if (showKH && chartAnnotationPluginLoaded) { 
        ...
        if (khPoints.length > 0) {
            ...
            options.scales.y2 = {
                ...
                min: khYMin,
                max: khYMax,
                grid: { drawOnChartArea: false, color: options.scales.y.grid.color },
...
function plotResistivityData(input) {
    ...
    const colorPalette = RESISTIVITY_PALETTES[selectedPaletteKey] ? RESISTIVITY_PALETTES[selectedPaletteKey].colors : RESISTIVITY_PALETTES["primary"].colors;
